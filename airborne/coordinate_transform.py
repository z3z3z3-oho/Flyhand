...
class CoordinateTransformNode(Node):
    def __init__(self):
        ...
        # ``T_cam2body`` describes the camera relative to the UAV body
        # (``base_link``).  The SO101 IK, however, expects points in the
        # hanging arm's local ``arm_base`` frame.  Keep this installation
        # transform here instead of altering the arm interface.
        #
        # The calibrated UAV/arm convention uses the same XYZ directions:
        # X forward, Y lateral, Z downward.  The SO101 base is 10 cm below
        # the UAV body origin in that convention.
        self.declare_parameter("arm_base_x_in_body_m", 0.0)
        self.declare_parameter("arm_base_y_in_body_m", 0.0)
        self.declare_parameter("arm_base_z_in_body_m", 0.10)
        ...
        self.arm_base_in_body = np.array(
            [
                float(self.get_parameter("arm_base_x_in_body_m").value),
                float(self.get_parameter("arm_base_y_in_body_m").value),
                float(self.get_parameter("arm_base_z_in_body_m").value),
            ],
            dtype=np.float64,
        )
        ...
        self.T_body2arm = self._make_body_to_arm_transform()
        ...
        self.get_logger().info(
            ...
            "output_frame=arm_base | "
            f"arm_base_in_body={self.arm_base_in_body.tolist()}"
        )

    def _make_body_to_arm_transform(self):
        """Return the fixed UAV-body to SO101-base translation.

        Both frames use the calibrated forward/lateral/down convention, so
        only the origin offset is required here.
        """
        rotation = np.eye(3, dtype=np.float64)
        transform = np.eye(4, dtype=np.float64)
        transform[:3, :3] = rotation
        # p_arm = R_body_to_arm * (p_body - p_arm_base_in_body)
        transform[:3, 3] = -rotation @ self.arm_base_in_body
        return transform
    ...
    def transform_point(self, point_cam):
        point_body = _transform_point(self.T_cam2body, point_cam)
        if point_body is None:
            return None
        return _transform_point(self.T_body2arm, point_body)
    ...
    def _detections_callback(self, msg):
        ...
        try:
            ...
            point_arm = self.transform_point(
                [
            ...
            pose_msg.pose.position.x = float(point_arm[0])
            pose_msg.pose.position.y = float(point_arm[1])
            pose_msg.pose.position.z = float(point_arm[2])
