...
class HarvestContext:
    ...
    coarse_gripper_open_sent:  bool        = False
    coarse_gripper_close_sent: bool        = False
    coarse_gripper_open_ts:    float       = 0.0
    coarse_gripper_close_ts:   float       = 0.0
    servo_move_sent:   bool               = False
    servo_move_ts:     float              = 0.0
...
class HarvestConfig:
    ...
    servo_timeout:          float = 10.0   # 视觉伺服超时
    servo_target_wait:      float = 1.0    # 进入 SERVOING 后等待新视觉坐标
    servo_min_move_wait:    float = 2.5    # SERVOING 第二次运动后最少等待
    pre_grasp_pause:        float = 0.8    # 到位后停顿再夹（秒），让机械臂稳定，抓取更自然
    gripper_close_wait:     float = 2.5    # 夹爪闭合后至少等待时间
    gripper_close_timeout:  float = 5.0    # 夹爪闭合超时
...
class HarvestStateMachine(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("servo_timeout",       10.0)
        self.declare_parameter("servo_target_wait",   1.0)
        self.declare_parameter("servo_min_move_wait", 2.5)
        self.declare_parameter("pre_grasp_pause",     0.8)
        self.declare_parameter("gripper_close_wait",  2.5)
        self.declare_parameter("gripper_close_timeout", 5.0)
        ...
        self.cfg = HarvestConfig(
            ...
            servo_timeout      = self.get_parameter("servo_timeout").value,
            servo_target_wait  = self.get_parameter("servo_target_wait").value,
            servo_min_move_wait= self.get_parameter("servo_min_move_wait").value,
            pre_grasp_pause    = self.get_parameter("pre_grasp_pause").value,
            gripper_close_wait = self.get_parameter("gripper_close_wait").value,
            gripper_close_timeout = self.get_parameter("gripper_close_timeout").value,
    ...
    def _s_coarse_move(self):
        ...
        # 到位后只进入一次 SERVOING，夹爪动作放到 SERVOING 内执行
    ...
    def _s_servoing(self):
        ...
        elapsed = time.time() - self._state_enter_time
        gripper_open_duration = 3.0
        fresh_target = self.ctx.last_detection_ts >= self._state_enter_time

        if not self.ctx.servo_move_sent:
            if self.ctx.target_pose is not None and elapsed >= self.cfg.servo_target_wait:
                self.pub_arm_pose.publish(self.ctx.target_pose)
                self.ctx.servo_move_sent = True
                self.ctx.servo_move_ts = time.time()
                self._target_locked = True
                self._arm_motion_done = False
                p = self.ctx.target_pose.pose.position
                target_age = time.time() - self.ctx.last_detection_ts
                self._log(
                    f"Servo once → Arm ({p.x:.3f}, {p.y:.3f}, {p.z:.3f}), "
                    f"target_age={target_age:.2f}s"
                )
            return

        servo_elapsed = time.time() - self.ctx.servo_move_ts
        servo_ready = servo_elapsed >= self.cfg.servo_min_move_wait
        servo_done_by_motion = self._arm_motion_done and servo_ready
        servo_done_by_aligned = self.ctx.servo_aligned and servo_ready
        servo_done_by_timeout = elapsed > self.cfg.servo_timeout
        servo_finished = servo_done_by_motion or servo_done_by_aligned or servo_done_by_timeout

        if servo_finished and not self.ctx.coarse_gripper_open_sent:
            self.pub_servo_en.publish(Bool(data=False))
            self.pub_gripper.publish(Bool(data=False))
            self.ctx.coarse_gripper_open_sent = True
            self.ctx.coarse_gripper_open_ts = time.time()
            if servo_done_by_aligned:
                self._log(f"Servo aligned ({elapsed:.1f}s), gripper open for {gripper_open_duration:.1f}s")
            elif servo_done_by_motion:
                self._log(f"Servo motion done ({servo_elapsed:.1f}s), gripper open for {gripper_open_duration:.1f}s")
            else:
                self._log(f"Servo timeout ({elapsed:.1f}s), gripper open for {gripper_open_duration:.1f}s")
            return

        if (
            self.ctx.coarse_gripper_open_sent
            and time.time() - self.ctx.coarse_gripper_open_ts >= gripper_open_duration
        ):
            self._log("SERVOING open delay done → GRIPPER_CLOSE")
    ...
    def _s_gripper_close(self):
        ...
        if not self.ctx.coarse_gripper_close_sent:
            self._log("Closing gripper...")
            self._arm_motion_done = False
            self.pub_gripper.publish(Bool(data=True))
            self.ctx.coarse_gripper_close_sent = True
            self.ctx.coarse_gripper_close_ts = time.time()
            return

        # 等待夹爪到位：即使 motion_done 很快到，也至少保留 gripper_close_wait 秒给球稳定夹住
        close_elapsed = time.time() - self.ctx.coarse_gripper_close_ts
        gripper_done = (
            (self._arm_motion_done and close_elapsed >= self.cfg.gripper_close_wait) or
            close_elapsed >= self.cfg.gripper_close_timeout
        )
        if gripper_done:
            if self._arm_motion_done:
    ...
    def _s_retracting(self):
        ...
        # 阶段1开始：抓取完成后回零，夹爪保持闭合
        if elapsed < 0.2:
            self.pub_arm_home.publish(Bool(data=True))
            self._log("Retracting arm, keeping gripper closed...")
        if elapsed < self.cfg.gripper_close_wait:
            self.pub_gripper.publish(Bool(data=True))
    ...
    def _cb_target(self, msg: PoseStamped):
        ...
        if (
            not self._target_locked
            or (self.state == State.SERVOING and not self.ctx.servo_move_sent)
        ):
    ...
    def _transition(self, new_state: State):
        old = self.state.name
        if new_state == State.COARSE_MOVE:
            self.ctx.coarse_gripper_open_sent = False
            self.ctx.coarse_gripper_close_sent = False
            self.ctx.coarse_gripper_open_ts = 0.0
            self.ctx.coarse_gripper_close_ts = 0.0
            self.ctx.servo_move_sent = False
            self.ctx.servo_move_ts = 0.0
        if new_state == State.SERVOING:
            self._target_locked = False
            self.ctx.servo_move_sent = False
            self.ctx.servo_move_ts = 0.0
            self.ctx.servo_aligned = False
            ...
            self.ctx.coarse_gripper_open_ts = 0.0
            self.ctx.coarse_gripper_close_ts = 0.0
        if new_state == State.GRIPPER_CLOSE:
            self.ctx.coarse_gripper_close_sent = False
            self.ctx.coarse_gripper_close_ts = 0.0
    ...
    def _target_to_world_hover(self, target: PoseStamped, standoff: float) -> np.ndarray:
        # /target/best is in ``arm_base``.  The calibrated UAV and arm
        # frames share XYZ directions; the arm base is 10 cm below the UAV.
        arm_base_in_body = np.array([0.0, 0.0, 0.10])
        tp = target.pose.position
        target_arm = np.array([tp.x, tp.y, tp.z])
        target_body = target_arm + arm_base_in_body
