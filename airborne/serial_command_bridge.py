...
* Hold frames: H,seq
* Gesture velocity JSON: {"mode":"servo","vx":...,"vy":...,"vz":...,"grip":...}
  Gripper aliases accepted for GESTURE only: grip / gripper_close / gripper.
* Gesture target JSON: {"mode":"target","x":...,"y":...,"z":...,"grip":...}
...
VALID_MODES = {"AUTO", "TELEOP", "GESTURE", "EEG", "HOLD", "STOW"}
COMMAND_ALIASES = {
    "start": "start",
    "hover": "hover",
    "next": "next",
...
class SerialCommandBridge(Node):
    def __init__(self):
        ...
        self.declare_parameter("eeg_command_topic", "/eeg/command")
        ...
        self.eeg_command_topic = str(
            self.get_parameter("eeg_command_topic").value
        )
        ...
        self.gesture_joint_pub = self.create_publisher(
            JointState, "/gesture/joint_target", 10
        )
        ...
        self.eeg_command_pub = self.create_publisher(
            String, self.eeg_command_topic, 10
        )
        ...
        self._gesture_pose_initialized = False
    ...
    def _reset_gesture_pose_filter(self):
        self._gesture_x = 0.251
        self._gesture_y = 0.0
        self._gesture_z = 0.142
        self._gesture_pose_initialized = False
    ...
    def _publish_gesture_reset(self, reason=""):
        ...
        if self._mode != "GESTURE":
            return
        self._reset_gesture_pose_filter()
    ...
    def _publish_gesture_gripper(self, command):
        if "grip" in command:
            grip = command["grip"]
        elif "gripper_close" in command:
            grip = command["gripper_close"]
    ...
    def _handle_gesture_pose(self, command):
        ...
        if not self._gesture_pose_initialized:
            self._gesture_x = target_x
            self._gesture_y = target_y
            self._gesture_z = target_z
            self._gesture_pose_initialized = True
        else:
    ...
    def _handle_gesture_joint(self, command):
        ...
        names = []
        values = []
        if any(key in command for key in ("shoulder_pan", "pan", "joint1")):
            names.append("shoulder_pan")
            values.append(float(first_value("shoulder_pan", "pan", "joint1")))
        if any(key in command for key in ("shoulder_lift", "lift", "joint2")):
            names.append("shoulder_lift")
            values.append(float(first_value("shoulder_lift", "lift", "joint2")))
        if any(key in command for key in ("wrist_flex", "wrist", "joint4")):
            names.append("wrist_flex")
            values.append(float(first_value("wrist_flex", "wrist", "joint4")))
        if any(key in command for key in ("joint4_offset", "wrist_offset")):
            names.append("wrist_flex_offset")
            values.append(float(first_value("joint4_offset", "wrist_offset")))
        if any(key in command for key in ("joint3_offset", "elbow_offset")):
            names.append("elbow_flex_offset")
            values.append(float(first_value("joint3_offset", "elbow_offset")))
        if any(key in command for key in ("elbow_flex", "elbow", "joint3")):
            names.append("elbow_flex")
            values.append(float(first_value("elbow_flex", "elbow", "joint3")))

        if not names:
            raise ValueError("gesture joint target contains no joints")

        if not all(math.isfinite(value) for value in values):
            raise ValueError("non-finite gesture joint target")
        ...
        message.header.stamp = self.get_clock().now().to_msg()
        message.name = names
        message.position = [math.radians(value) for value in values]
        self.gesture_joint_pub.publish(message)
    ...
    def _handle_gesture_json(self, raw):
        ...
        if gesture_type in {"joint", "joints", "joint_target"} or any(
            key in command for key in (
                "shoulder_pan", "pan", "joint1",
                "shoulder_lift", "lift", "joint2",
                "elbow_flex", "elbow", "joint3",
                "joint3_offset", "elbow_offset",
                "wrist_flex", "wrist", "joint4", "joint4_offset",
                "wrist_offset",
            )
        ):
            self._handle_gesture_joint(command)
        elif gesture_type in {"servo", "gesture", "velocity", "vel"} or any(
            key in command for key in ("vx", "vy", "vz")
        ):
            self._handle_gesture_velocity(command)
        elif gesture_type in {"target", "pose", "position", "pos"} or all(
            key in command for key in ("x", "y", "z")
        ):
            if not all(key in command for key in ("x", "y", "z")):
                raise ValueError("gesture target requires x/y/z")
    ...
    def _handle_eeg(self, body):
        parts = [part.strip().upper() for part in body.split(",")]
        if len(parts) < 2 or parts[0] != "EEG":
            raise ValueError("invalid EEG frame")
        command = parts[1]
        if command not in {
            "MOVE_FWD_DOWN",
            "FWD_DOWN",
            ...
            raise ValueError(f"invalid EEG command: {command}")
        if self._mode != "EEG" and command not in {"HOLD", "STOP"}:
            self._send_feedback(f"ERR,MODE,{self._mode}")
            return
        self.eeg_command_pub.publish(String(data=command))
        self._send_feedback(f"OK,EEG,{command}")
    ...
    def _handle_joint(self, body):
        ...
        if self._mode not in {"TELEOP", "EEG"}:
            self._send_feedback(f"ERR,MODE,{self._mode}")
            return
        ...
        if now - self._last_joint_ack_time >= 1.0:
            self._send_feedback(f"ACK,J,{sequence},{self._mode}")
