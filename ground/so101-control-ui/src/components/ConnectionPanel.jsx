...
const DEFAULTS = {
  leader_port: 'COM6',
  radio_port: 'COM7',
  leader_id: 'ground_leader',
  camera_index: 20,
  baudrate: 57600,
  hz: 30,
  gesture_speed: 0.9,
  hand_model: 'C:\\Users\\Lenovo\\Desktop\\SO101_3_modes\\ground\\hand_landmarker.task',
  pose_model: 'C:\\Users\\Lenovo\\Desktop\\SO101_3_modes\\ground\\pose_landmarker_full.task',
}
...
export function ConnectionPanel({ connected, connecting, onChanged }) {
  const [config, setConfig] = useState(() => {
    ...
    try {
      return { ...DEFAULTS, ...JSON.parse(saved), pose_model: DEFAULTS.pose_model }
    } catch {
      return DEFAULTS
    }
  })
    ...
    try {
      if (connected || connecting) {
        ...
        const payload = {
          ...DEFAULTS,
          ...config,
          leader_port: String(config.leader_port || DEFAULTS.leader_port).trim(),
          radio_port: String(config.radio_port || DEFAULTS.radio_port).trim(),
          leader_id: String(config.leader_id || DEFAULTS.leader_id).trim(),
          hand_model: String(config.hand_model || DEFAULTS.hand_model).trim(),
          pose_model: String(config.pose_model || DEFAULTS.pose_model).trim(),
          camera_index: Number.isFinite(Number(config.camera_index)) ? Number(config.camera_index) : DEFAULTS.camera_index,
          baudrate: Number.isFinite(Number(config.baudrate)) ? Number(config.baudrate) : DEFAULTS.baudrate,
          hz: Number.isFinite(Number(config.hz)) ? Number(config.hz) : DEFAULTS.hz,
          gesture_speed: Number.isFinite(Number(config.gesture_speed)) ? Number(config.gesture_speed) : DEFAULTS.gesture_speed,
        }
        await connectController(payload)
      }
      await onChanged()
    } catch (requestError) {
      ...
      <div className="p-6">
        ...
        {expanded ? (
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field label="姿态模型路径" value={config.pose_model} onChange={update('pose_model')} />
          </div>
        ) : null}
