...
import ground_so101_sender as sender_module
...
DEFAULT_POSE_MODEL = ROOT / "pose_landmarker_full.task"
BUILD_ID = getattr(sender_module, "BUILD_ID", "unknown-build")
PROTOCOL_BUILD_ID = getattr(sender_module, "PROTOCOL_BUILD_ID", BUILD_ID)
...
class ControlRuntime:
    ...
    def gesture_preview(self) -> dict[str, Any]:
        getter = getattr(self.worker, "get_gesture_preview", None)
        if getter is None:
            return {
                "jpeg": None,
                "status": "",
    ...
    def connect(self, config: dict[str, Any]) -> None:
        ...
        pose_model = Path(str(config.get("pose_model", ""))).expanduser()
        if pose_model and not pose_model.is_file():
            raise RuntimeError(f"姿态模型不存在: {pose_model}")
...
class ConsoleCommandBridge:
    """Allow local single-key control without exposing the mapping in the web UI."""
    ...
    def __init__(self, control_runtime: ControlRuntime) -> None:
        self.runtime = control_runtime
        self.stop_event = threading.Event()
        self.thread = threading.Thread(
            target=self._run,
            name="so101-console-command-bridge",
            daemon=True,
        )
    ...
    def _run(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            if not key:
                time.sleep(0.05)
                continue
            command = self.KEY_TO_COMMAND.get(key.lower())
            if command is None:
                continue
            try:
                self.runtime.command(command)
            except RuntimeError as exc:
                runtime._append_log("warn", f"本地辅助控制无法执行: {exc}")
            else:
                runtime._append_log("info", "本地辅助控制已触发")
                if key.lower() == "q":
                    self.stop()


console_bridge = ConsoleCommandBridge(runtime)
